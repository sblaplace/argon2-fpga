...
// Large-m' known-answer fill: p=1, m'=128 KiB, t=3, through the full
// argon2_fill_axi → AXI4-MM → tb_axi_ram stack. Same structure as
// tb_argon2_axi but 48x the block count of the small KAT — enough to
// exercise the scheduling paths that only appear at scale:
//
//   * early dependent-ref reads whose target sits in the 32-beat write
//     FIFO (skipped, serial fallback) or is the block being drained,
//   * early dest-xor reads crossing 32-block segment boundaries,
//   * multiple address-generation windows per pass (argon2i),
//   * write-FIFO wraparound under sustained drain.
//
// Vectors come from ref/ via tests/dump_vectors.py (gen/bigfill_*).
`timescale 1ns / 1ps

module tb_argon2_axi_big #(
    parameter int N_P = 1   // parallel P units in the compression G
);
    localparam int NBLK   = 128;
    ...
    initial begin
        ...
        run_job(2'd1, "gen/bigfill_i_init.hex",  "gen/bigfill_i_exp.hex",  "argon2i");
        run_job(2'd0, "gen/bigfill_d_init.hex",  "gen/bigfill_d_exp.hex",  "argon2d");
        run_job(2'd2, "gen/bigfill_id_init.hex", "gen/bigfill_id_exp.hex", "argon2id");

        if (errors == 0) begin
            $display("tb_argon2_axi_big PASS");
            $finish;
        end else begin
            $display("tb_argon2_axi_big FAIL (%0d)", errors);
            $fatal(1);
        end
    end
endmodule
